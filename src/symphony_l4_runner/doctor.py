...
def run_doctor(env: Mapping[str, str] | None = None, stdout: TextIO | None = None) -> int:
    """Run the local no-secret live-mode preflight."""

    checks = evaluate_environment(os.environ if env is None else env)
    ...
    blocker_count = sum(1 for check in checks if check.blocking)
    if blocker_count:
